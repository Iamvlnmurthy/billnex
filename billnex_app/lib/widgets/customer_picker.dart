...
class _CustomerPickerSheetState extends State<_CustomerPickerSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final matches = widget.state.customers
        .where((c) => c.name.toLowerCase().contains(_q.toLowerCase()) || c.mobile.contains(_q))
        .toList();
            ...
            : Column(mainAxisSize: MainAxisSize.min, children: [
                Padding(
                  ...
                  child: Column(children: [
                    Row(children: [
                      const Expanded(child: Text('Select customer', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800))),
                ...
                Flexible(
                  child: matches.isEmpty
                      ? Padding(
                          ...
                          child: Column(children: [
                            Text(_q.isEmpty ? 'No customers yet' : 'No match', style: TextStyle(color: bx.muted, fontWeight: FontWeight.w600)),
                            const SizedBox(height: 10),
                            FilledButton.icon(onPressed: () => setState(() => _adding = true), icon: const Icon(Icons.person_add_alt, size: 18), label: Text(_q.isEmpty ? 'Add customer' : 'Add "$_q"')),
                          ]),
                        )
                      : ListView.separated(
                          shrinkWrap: true,
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            return ListTile(
                              leading: CircleAvatar(backgroundColor: bx.brand.withValues(alpha: 0.12), child: Text(c.name.characters.first.toUpperCase(), style: TextStyle(color: bx.brand, fontWeight: FontWeight.w800))),
                              title: Text(c.name, style: const TextStyle(fontWeight: FontWeight.w700)),
                              subtitle: Text(c.mobile.isEmpty ? 'No mobile' : c.mobile),
                              trailing: bal > 0
                                  ? Text('${money(bal)} due', style: TextStyle(color: bx.danger, fontWeight: FontWeight.w700, fontSize: 12.5))
                                  : Text('Settled', style: TextStyle(color: bx.pos, fontWeight: FontWeight.w700, fontSize: 12.5)),
