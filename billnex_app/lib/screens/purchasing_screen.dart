...
class PurchasingScreen extends StatelessWidget {
        ...
        children: [
          ConstrainedBox(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
              PageHeader('Purchasing & Suppliers',
              ...
              if (suppliers.isEmpty)
                Card(child: Padding(padding: const EdgeInsets.symmetric(vertical: 44), child: Column(children: [
                  ...
                  Text('No suppliers yet', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: bx.muted)),
  ...
  Widget _row(BuildContext context, Supplier s, bool first) {
    ...
    return InkWell(
      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => SupplierDetailScreen(state: state, supplierId: s.id))),
      child: Container(
        decoration: BoxDecoration(border: first ? null : Border(top: BorderSide(color: bx.border))),
        padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
        child: Row(children: [
          CircleAvatar(radius: 20, backgroundColor: bx.brand.withValues(alpha: 0.12), child: Text(s.name.characters.first.toUpperCase(), style: TextStyle(color: bx.brand, fontWeight: FontWeight.w800))),
          const SizedBox(width: 12),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(s.name, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700), overflow: TextOverflow.ellipsis),
              const SizedBox(height: 2),
              Text(s.phone.isEmpty ? (s.gstin ?? 'No contact') : s.phone, style: TextStyle(fontSize: 12, color: bx.muted)),
            ]),
          ),
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            Text(pay > 0 ? money(pay) : 'Settled', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w800, color: pay > 0 ? bx.warn : bx.pos)),
    ...
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 10),
          TextField(controller: gstin, decoration: const InputDecoration(labelText: 'GSTIN (optional)', border: OutlineInputBorder())),
          const SizedBox(height: 16),
          FilledButton(onPressed: () => Navigator.pop(ctx, true), style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14)), child: const Text('Save supplier')),
        ]),
      ),
    );
...
class _PurchaseSheetState extends State<_PurchaseSheet> {
  ...
  double get _total => _lines.fold<double>(0, (a, l) => a + l.amount) * 1.05;
    ...
    return Padding(
      ...
      child: SingleChildScrollView(
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 10),
          TextField(controller: _ref, onChanged: (_) => setState(() {}), decoration: InputDecoration(
            labelText: "Supplier invoice no.",
            border: const OutlineInputBorder(),
            errorText: dup ? 'Duplicate invoice for this supplier' : null,
          )),
          const SizedBox(height: 12),
          Row(children: [
          ...
          for (int i = 0; i < _lines.length; i++)
            Padding(
              ...
              child: Row(children: [
                Expanded(child: Text(_lines[i].sku, style: const TextStyle(fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis)),
                Text('${_lines[i].qty.toStringAsFixed(0)} × ${money(_lines[i].rate)}', style: TextStyle(color: bx.muted, fontSize: 13)),
                const SizedBox(width: 8),
                Text(money(_lines[i].amount), style: const TextStyle(fontWeight: FontWeight.w700)),
                IconButton(visualDensity: VisualDensity.compact, onPressed: () => setState(() => _lines.removeAt(i)), icon: Icon(Icons.close, size: 16, color: bx.faint)),
              ]),
            ),
          if (_lines.isEmpty) Padding(padding: const EdgeInsets.symmetric(vertical: 8), child: Text('No items yet', style: TextStyle(color: bx.faint))),
          const Divider(),
          Row(children: [
            const Text('Total (incl. 5% GST)', style: TextStyle(fontWeight: FontWeight.w700)),
            const Spacer(),
          ...
          FilledButton.icon(
            onPressed: _lines.isEmpty ? null : _record,
            icon: const Icon(Icons.check, size: 18),
            label: const Text('Record purchase & stock-in'),
  ...
  Future<Supplier?> _pickSupplier(BuildContext context) => showModalBottomSheet<Supplier>(
        ...
        builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
          for (final s in widget.state.suppliers)
            ListTile(title: Text(s.name), subtitle: Text('${money(widget.state.payableOf(s.id))} payable'), onTap: () => Navigator.pop(ctx, s)),
          ListTile(leading: const Icon(Icons.person_add_alt), title: const Text('New supplier'), onTap: () async {
  ...
  Future<void> _addLine() async {
        ...
        child: SizedBox(
          ...
          child: ListView(children: [
            for (final it in items)
              ListTile(title: Text(it.name), subtitle: Text('on hand ${it.qty.toStringAsFixed(0)} · cost ${money(it.cost)}'), onTap: () => Navigator.pop(ctx, it)),
          ]),
        ),
      ),
    );
    ...
    final rate = TextEditingController(text: picked.cost.toStringAsFixed(0));
    final ok = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      showDragHandle: true,
      builder: (ctx) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: TextField(controller: qty, autofocus: true, keyboardType: TextInputType.number, decoration: InputDecoration(labelText: 'Qty (${picked.unit})', border: const OutlineInputBorder()))),
            const SizedBox(width: 10),
            Expanded(child: TextField(controller: rate, keyboardType: TextInputType.number, decoration: const InputDecoration(prefixText: '₹ ', labelText: 'Rate', border: OutlineInputBorder()))),
          ]),
          const SizedBox(height: 16),
          FilledButton(onPressed: () => Navigator.pop(ctx, true), style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14)), child: const Text('Add line')),
        ]),
      ),
    );
    if (ok == true) {
      setState(() => _lines.add(PurchaseLine(picked.sku, double.tryParse(qty.text) ?? 0, double.tryParse(rate.text) ?? 0)));
    }
  }
  ...
  void _record() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Purchase recorded · ${_lines.length} items stocked-in ✓')));
  }
}
...
class SupplierDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
      animation: state,
      builder: (context, _) {
        final s = state.suppliers.firstWhere((x) => x.id == supplierId);
        final pay = state.payableOf(s.id);
        ...
        return Scaffold(
          ...
          body: ListView(padding: const EdgeInsets.fromLTRB(16, 12, 16, 100), children: [
            Card(child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text('PAYABLE BALANCE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 0.4, color: bx.faint)),
                const SizedBox(height: 6),
                Text(money(pay), style: TextStyle(fontSize: 30, fontWeight: FontWeight.w800, color: pay > 0 ? bx.warn : bx.pos)),
                if (s.phone.isNotEmpty) ...[const SizedBox(height: 4), Text(s.phone, style: TextStyle(fontSize: 13, color: bx.muted))],
                const SizedBox(height: 14),
                FilledButton.icon(
                  onPressed: pay <= 0 ? null : () => _pay(context, s, pay),
            ...
            const SizedBox(height: 16),
            Padding(padding: const EdgeInsets.only(left: 2, bottom: 8), child: Text('PURCHASES', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, letterSpacing: 0.4, color: bx.faint))),
            if (purchases.isEmpty)
              Card(child: Padding(padding: const EdgeInsets.symmetric(vertical: 28), child: Center(child: Text('No purchases', style: TextStyle(color: bx.muted)))))
            else
                  ...
                  Container(
                    ...
                    child: Row(children: [
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        Text('${purchases[i].supplierRef.isEmpty ? 'no ref' : purchases[i].supplierRef} · ${purchases[i].dateLabel} · ${purchases[i].lines.length} items', style: TextStyle(fontSize: 12, color: bx.muted)),
                      ])),
  ...
  Future<void> _pay(BuildContext context, Supplier s, double due) async {
    final amt = TextEditingController(text: due.round().toString());
    String mode = 'Cash';
    final ok = await showModalBottomSheet<bool>(
      context: context,
      isScrollControlled: true,
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setSt) => Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          const SizedBox(height: 12),
          TextField(controller: amt, keyboardType: TextInputType.number, decoration: const InputDecoration(prefixText: '₹ ', labelText: 'Amount', border: OutlineInputBorder())),
          const SizedBox(height: 12),
          Wrap(spacing: 8, children: [for (final m in ['Cash', 'UPI', 'Bank']) ChoiceChip(label: Text(m), selected: mode == m, onSelected: (_) => setSt(() => mode = m))]),
          const SizedBox(height: 16),
          FilledButton(onPressed: () => Navigator.pop(ctx, true), style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 14)), child: const Text('Record payment')),
        ]),
      )),
    );
    if (ok == true) {
      ...
      if (v > 0) {
        ...
        if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Paid ${money(v)} to ${s.name} ✓')));
      }
    }
  }
}
