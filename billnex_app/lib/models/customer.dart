...
class Customer {
  ...
  const Customer({required this.id, required this.name, required this.mobile, this.gstin, this.creditLimit = 0, this.consent = false});

  Customer copyWith({String? name, String? mobile, String? gstin, double? creditLimit, bool? consent}) =>
      Customer(id: id, name: name ?? this.name, mobile: mobile ?? this.mobile, gstin: gstin ?? this.gstin, creditLimit: creditLimit ?? this.creditLimit, consent: consent ?? this.consent);

  Map<String, dynamic> toJson() => {'id': id, 'n': name, 'm': mobile, 'g': gstin, 'cl': creditLimit, 'c': consent};

  factory Customer.fromJson(Map<String, dynamic> j) =>
      Customer(id: j['id'] as String, name: j['n'] as String, mobile: (j['m'] ?? '') as String, gstin: j['g'] as String?, creditLimit: (j['cl'] as num?)?.toDouble() ?? 0, consent: j['c'] == true);
}
