...
class BusinessProfile {
  ...
  const BusinessProfile({required this.bizType, required this.shopName, this.owner = '', this.phone = '', this.gstin, this.address = '', this.stateCode = '36', this.taxInclusive = true});

  bool get hasGst => (gstin ?? '').trim().length >= 15;

  BusinessProfile copyWith({String? shopName, String? owner, String? phone, String? gstin, String? address, String? stateCode, bool? taxInclusive}) => BusinessProfile(
    bizType: bizType,
  ...
  Map<String, dynamic> toJson() => {'type': bizType, 'shop': shopName, 'owner': owner, 'phone': phone, 'gstin': gstin, 'addr': address, 'state': stateCode, 'taxIncl': taxInclusive};
